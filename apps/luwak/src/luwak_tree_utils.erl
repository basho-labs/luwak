...
%% equal length subseq
longest_divisable_subtree([{NameA,LA}|NodesA], [{NameB,LB}|NodesB], LengthA, LengthB, AccA, AccB) when LA + LengthA == LB + LengthB ->
  longest_divisable_subtree(NodesA, NodesB, LengthA+LA, LengthB+LB, [{NameA,LA}|AccA], [{NameB,LB}|AccB]);
%% left is larger
longest_divisable_subtree([{NameA,LA}|NodesA], [{NameB,LB}|NodesB], LengthA, LengthB, AccA, AccB) when LA + LengthA > LB + LengthB ->
  longest_divisable_subtree([{NameA,LA}|NodesA], NodesB, LengthA, LengthB+LB, AccA, [{NameB,LB}|AccB]);
%% right is larger
longest_divisable_subtree([{NameA,LA}|NodesA], [{NameB,LB}|NodesB], LengthA, LengthB, AccA, AccB) when LA + LengthA < LB + LengthB ->
  longest_divisable_subtree(NodesA, [{NameB,LB}|NodesB], LengthA+LA, LengthB, [{NameA,LA}|AccA], AccB).
  

shortest_subtree_split(NodesA, NodesB, StartA, StartB) ->
  shortest_subtree_split(NodesA, NodesB, StartA, StartB, [], []).
  
shortest_subtree_split(NodesA, NodesB, StartA, StartB, AccA, AccB) when length(NodesA) == 0; length(NodesB) == 0; StartA == StartB ->
  {{lists:reverse(AccA),NodesA},{lists:reverse(AccB),NodesB}};
shortest_subtree_split([E={_NA,LA}|NodesA], NodesB, StartA, StartB, AccA, AccB) when StartA < StartB ->
  shortest_subtree_split(NodesA, NodesB, LA+StartA, StartB, [E|AccA], AccB);
shortest_subtree_split(NodesA, [E={_NB,LB}|NodesB], StartA, StartB, AccA, AccB) when StartA > StartB ->
...
split_at_length(Tail = [{_Name,L}|_], Length, AccLen, Acc) when AccLen+L > Length ->
  {lists:reverse(Acc), Tail};
...
five_way_split(TreePos, Nodes, InsertPos, Blocks) ->
  ...
  {{OverlapHeadNode, TailNode2}, {OverlapHeadBlocks, TailBlocks1}} = shortest_subtree_split(TailNode1, Blocks, 0, NegativeOverlap),
  ?debugFmt("2: ~p~n", [{{OverlapHeadNode, TailNode2}, {OverlapHeadBlocks, TailBlocks1}}]),
  {{MiddleNode, TailNode3}, {MiddleBlocks, TailBlocks2}} = longest_divisable_subtree(TailNode2, TailBlocks1),
  ?debugFmt("3: ~p~n", [{{MiddleNode, TailNode3}, {MiddleBlocks, TailBlocks2}}]),
