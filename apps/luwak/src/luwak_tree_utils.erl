...
shortest_subtree_split([E={NA,LA}|NodesA], NodesB, StartA, StartB, AccA, AccB) when StartA < StartB ->
...
split_at_length(Tail = [{Name,L}|_], Length, AccLen, Acc) when AccLen+L > Length ->
  {lists:reverse(Acc), Tail};
